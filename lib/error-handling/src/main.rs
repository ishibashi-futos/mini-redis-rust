fn main() {
...

    {
        ...
        let display_weather = |weather: Result<Weather, String>| {
            match weather {
        ...
        let weather = get_weather(LatLng { lat: 10f64, lng: 100f64 });
        display_weather(weather);
        let weather = get_weather(LatLng { lat: 25f64, lng: 35f64 });
        display_weather(weather);
        let weather = get_weather(LatLng { lat: 101f64, lng: 10f64 });
        display_weather(weather);

        let result = get_weather(LatLng { lat: 10f64, lng: 100f64 });
        // Resultにあるよく使うメソッド
        ...
        let result = get_weather(LatLng { lat: 101f64, lng: 10f64 });
        ...
        let result = get_weather(LatLng { lat: 101f64, lng: 10f64 });
        assert!(result.is_err());
        assert_eq!(Weather::Cloudy, result.unwrap_or_else(|_err| Weather::Cloudy));

        // unwrap / expectは成功だった場合のみ値を返し、エラーだった場合はpanicを起こしてしまう
        let _result = get_weather(LatLng { lat: 10f64, lng: 100f64 }).unwrap();
        // expectの場合はmessageを指定することができる
        let _result = get_weather(LatLng { lat: 10f64, lng: 100f64 }).expect("expected!");

        // 以下のメソッドは、Resultの値を`借用`できる
        let result = get_weather(LatLng { lat: 25f64, lng: 35f64 });
        // as_refはResult<T, E>をResult<&T, &E>の共用参照に変換する
        assert_eq!(Ok(&Weather::Cloudy), result.as_ref());
        let mut result = get_weather(LatLng { lat: 101f64, lng: 10f64 });
        // as_mutは可変参照に変換する
